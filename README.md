...
应用可以完全自部署。账户、词库、卡片和复习记录保存在自己的 SQLite 数据库中；AI 默认连接 DeepSeek，登录用户也可以加密保存自己的 DeepSeek、OpenAI、Gemini、Claude、通义千问、Kimi、智谱 GLM 或 Grok Key，统一通过服务商的 OpenAI 兼容接口调用。没有配置 AI Key 时，基础词典查询仍然可用。
